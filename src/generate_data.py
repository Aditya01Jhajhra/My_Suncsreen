...
def generate_products(n_products=850):
...
def generate_customers(n_customers=3500):
    ...
    for i in range(n_customers):
        city = np.random.choice(cities, p=[0.12, 0.12, 0.10, 0.08, 0.08, 0.07, 0.07, 0.06, 0.06, 0.07])
...
def generate_orders(products_df, customers_df, n_orders=12000):
...
def generate_competitors():
    ...
    competitors = [
        ('Minimalist', 'SPF 50', 50, 499, 18000, 4.4, 'Gel', 'Silicone-free, Niacinamide, Zinc Oxide'),
        ('Neutrogena', 'Ultra Sheer Dry-Touch', 50, 649, 25000, 4.3, 'Lotion', 'Avobenzone, Homosalate, Octisalate'),
        ('Supergoop!', 'Unseen Sunscreen', 40, 3199, 2500, 4.6, 'Gel', 'Ethylhexyl Methoxycinnamate, Zinc Oxide'),
        ('La Roche Posay', 'Anthelios Age Correct', 50, 899, 12000, 4.5, 'Cream', 'Mexoryl XL, Octocrylene, Titanium Dioxide'),
        ('ISDIN', 'UVelix Dry Touch', 50, 799, 8500, 4.4, 'Lotion', 'Mexoryl SX, Avobenzone'),
        ('Biore', 'UV Aqua Rich Watery', 50, 549, 28000, 4.2, 'Gel', 'Ethylhexyl Methoxycinnamate, Octocrylene'),
        ('Himalaya', 'UV Protection Face Cream', 50, 399, 15000, 4.1, 'Cream', 'Zinc Oxide, Turmeric Extract'),
        ('Vaseline', 'Intensive Care SPF 50', 50, 249, 35000, 3.9, 'Lotion', 'Oxybenzone, Avobenzone'),
        ('CeraVe', 'Hydrating Mineral Sunscreen', 34, 750, 5000, 4.3, 'Cream', 'Zinc Oxide, Hyaluronic Acid'),
        ("Sun'bly", "Daily Care Sunscreen", 50, 450, 4200, 4.2, "Gel", "Zinc Oxide, Centella Asiatica")
    ]
...
def generate_city_metrics():
    ...
    for city in cities:
        ...
        city_metrics.append({
            ...
            'e_commerce_penetration': round(0.35 + (5 - rank) * 0.12, 2),
...
def generate_marketing_channels():
...
def generate_customer_insights(customers_df):
    ...
    for _, c in customers_df.iterrows():
        ...
        customer_insights.append({
            ...
            'spf_preference': int(np.random.choice([30, 50, 'NA'], p=[0.35, 0.60, 0.05])),
